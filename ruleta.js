document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ocultar la ruleta hasta que se ponga la contraseña correcta
    ruletaContainer.style.display = 'none';

    // Función para verificar la contraseña
    const verificarPassword = () => {
        if (passInput.value === "aula2026") {
            modal.style.display = 'none'; // Quitar el modal
            ruletaContainer.style.display = 'flex'; // Mostrar la ruleta
            iniciarRuleta(); // Arrancar la conexión a Firebase y el Canvas
        } else {
            errorMsg.classList.remove('hidden'); // Mostrar error
            passInput.value = ''; // Limpiar el campo
        }
    };

    // Escuchar el clic en el botón de "Acceder"
    btnSubmit.addEventListener('click', verificarPassword);

    // Permitir dar "Enter" en el teclado sin tener que usar el mouse
    passInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') verificarPassword();
    });

    // --- LÓGICA PRINCIPAL DE LA RULETA (Solo se ejecuta si hay acceso) ---
        ...
        ref.on('value', (snapshot) => {
            ...
            if (data) {
                Object.keys(data).forEach(key => {
                    participantes.push({ id: key, ...data[key] });
                });
            }
